...
# Polly 음성 프리셋 키 → Polly VoiceId 매핑
TTS_POLLY_VOICES = {
...
def _get_polly_client():
    ...
    try:
        ...
        raise RuntimeError("❌ boto3가 설치되어 있지 않습니다. `pip install boto3` 후 실행하세요.") from e
...
def synthesize_lines_polly(
    ...
    """SSML 라인 배열을 Polly로 합성하여 파일 저장"""
    if not ssml_lines:
        return []

    os.makedirs(out_dir, exist_ok=True)
    voice_id = TTS_POLLY_VOICES.get(voice_key) or "Seoyeon"
    ...
    for i, ssml in enumerate(ssml_lines):
        ...
        try:
            ...
            print("⚠️ Polly 합성 실패:", e)
            _write_silence_mp3(out_path, 0.6)
...
def mix_polly(chunk_paths: List[str], out_path: str) -> str:
    """Polly 조각 MP3를 ffmpeg concat으로 이어붙이기"""
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
